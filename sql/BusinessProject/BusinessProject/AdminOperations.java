...
public class AdminOperations {
    ...
    public void addUser(User user) {
        String sql = "INSERT INTO User (name, surname, phoneNumber, salary, isAdmin, birth, department, email, remainingLeaveDays, monthlyPerformance, bonusSalary, isActive) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void deleteUser(int userId) {
        ...
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            // Kullanıcının ID'sini set ediyoruz
            statement.setInt(1, userId);

            // Veritabanında güncelleme yapıyoruz
            int rowsAffected = statement.executeUpdate();
            if (rowsAffected > 0) {
                System.out.println("Kullanıcı başarıyla silindi (aktif durumu 'false' yapıldı).");
            } else {
    ...
    public static void main(String[] args) {
        ...
        //User user1 = new User("Uğur", "Güdükbay","1234567",1000, true, "1990-05-15", "CS", "UGURGUDUKBAY@ICLOUD.COM", 40, true);
        //ao.addUser(user1);//Ekleme yapıldı yapmadıysanız bir kere runlayın beyler
        //User user2 = new User("BILL", "GATES","9876543",1000, true, "1980-05-15", "CS", "BILLGATES@ICLOUD.COM", 40, true);
        //ao.addUser(user2);//Ekleme yapıldı yapmadıysanız bir kere runlayın beyler
        //User user3 = new User("Elon", "Musk","5891238",100, false, "2000-05-15", "CS", "ELONMUSK@ICLOUD.COM", 40, true);
